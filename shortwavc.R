...
aspectgrid1[aspectgrid <= 180 & aspectgrid != -1] <- 180 - aspectgrid
aspectgrid1[aspectgrid > 180] <- 540 - aspectgrid
...
for (daynumber in daystart:dayend)
	...
	elseif ((-1 * tan(latitude) * tan(decl)) > 1.0) sunrise <- acos(1)
	...
	while (hourangle < sunset)
		...
		elseif (cos(hourangle) < test) solaraz <- pi - asin(cos(decl)*sin(hourangle)/cos(solaralt))
		elseif (test == cos(hourangle) & hourangle >= 0) solaraz <- pi/2
		elseif (test == cos(hourangle) & hourangle < 0) solaraz <- -1 * pi / 2
		...
		azirad <- azi * deg2rad
		solaraltrad <- solaraltdeg * deg2rad
		sungrid = hillshade(slopegrid, aspectgridr, solaraltrad, azirad)
		...
		initalgrid <- wattsgrid + initalgrid
